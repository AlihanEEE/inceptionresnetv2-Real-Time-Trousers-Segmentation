...
class Epoch:
    ...
    #         x = x[0]
    #     if x.ndim == 3:
    #         final_image = np.zeros((x.shape[1], x.shape[2], 3))
    #         for index, x_s in enumerate(x):
    #             x1 = self.one_channel_to_three(x_s) * 255.0
    #             x1 = np.where(x1 == 255.0, index + 1, 0)
    #             final_image += x1
